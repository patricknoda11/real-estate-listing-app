...
router.post("/login", validRegisterationInfo, async (req, res) => {
  ...
  try {
    ...
    const agentToken = jwtTokenGenerator(agent.rows[0].agentId);

    res.status(200).json({ jwt_token: agentToken });
  } catch (_error) {
    res
...
router.post("/registeration", validRegisterationInfo, async (req, res) => {
  ...
  try {
    ...
    if (agent.rows.length !== 0) {
      res
        .status(AGENT_ALREADY_EXISTS_CODE)
        .json({ error: AGENT_ALREADY_EXISTS_MESSAGE });
    }
    ...
    // Create new agent:
    const newAgentId = await pool.query(CREATE_AGENT_QUERY, [
      phoneNumber,
      email,
      encryptedPassword,
      name,
    ...
    const agentToken = jwtTokenGenerator(newAgentId.agentId);
